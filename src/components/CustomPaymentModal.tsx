...
/**
 * Importing this module already starts the Stripe.js download, since
 * loadStripe runs at module scope. Kept as a named export so the order form can
 * trigger that download ahead of the payment step without rendering anything.
 */
...
interface CustomPaymentModalProps {
  ...
  productName: string;
  orderId: string | null;
  /** Raw amounts for analytics — `amount` is already formatted for display. */
  value: number;
  valueUsd: number;
  currency: string;
  onSuccess: () => void;
